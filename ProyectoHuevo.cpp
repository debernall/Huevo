...
int main () {
	...
	double t_muestreo = 100000;								//Tiempo de muestreo máximo (PENDIENTE)
	int tf = int(t_muestreo/dt);								//Número máximo de intervalos de tiempo
	
	//CONDICIONES INICIALES
	double T0_Huevo=40;									//Temperatura inicial Huevo
		...
		for (int i=0;i<n;i++){
			for (int j=0; j<n;j++){
				for (int k=0; k<n; k++){
					...
					else {							//Temperatura futura por medio del método diferencias finitas - balance de energía
						Tf[i][j][k]=Fo_i*((T[i][j][k]*((1/Fo_i)-6))+T[i+1][j][k]+T[i-1][j][k]+T[i][j+1][k]+T[i][j-1][k]+T[i][j][k+1]+T[i][j][k-1]);	
					}							//FALTA INCLUIR EL TÉRMINO DE GENERACIÓN, CONVECCIÓN Y MATRIZ DE GENERACIÓN.
				}
			}
		}
